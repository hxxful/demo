...
)
{
	...
	case DEV_MMC :
        ...
        result = EMMC_ReadMultiBlocks(buff, sector, MyEMMC_CardInfo.CardBlockSize, count);
		// translate the reslut code here
        if(EMMC_OK != result) {
            res = RES_ERROR;
            return res;
...
)
{
	...
	case DEV_MMC :
        ...
        result = EMMC_WriteMultiBlocks((uint8_t *)buff, sector, MyEMMC_CardInfo.CardBlockSize, count);
		// translate the reslut code here
        if(EMMC_OK != result) {
            res = RES_ERROR;
            return res;
